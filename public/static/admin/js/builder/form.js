...
jQuery(document).ready(function() {
    ...
    $('.js-jcrop-interface').each(function () {
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            if (res.code == 1) {
                                ...
                                Yxin.notify('上传失败，请重新上传', 'warning');
                            }
                        }
                    }).fail(function(res) {
                        Yxin.loading('hide');
        ...
        $jcrop_cut_btn.click(function () {
            ...
            if ($cut_value != '') {
                ...
                .fail(function(res) {
                    Yxin.loading('hide');
                    Yxin.notify($(res.responseText).find('h1').text() || '请求失败~', 'danger');
                });
            } else {
                Yxin.notify('请选择要裁剪的大小', 'warning');
            }
        });
    ...
    $('.js-ueditor').each(function () {
        ...
        ueditors[ueditor_name] = UE.getEditor(ueditor_name, {
            ...
            serverUrl: yxin.ueditor_upload_url
        });
    });
    ...
    $('.js-wangeditor').each(function () {
        ...
        // 上传图片地址
        wangeditors[wangeditor_name].config.uploadImgUrl = yxin.wangeditor_upload_url;
        // 允许上传图片后缀
    ...
    $('.js-upload-file,.js-upload-files').each(function () {
        ...
        var uploader = WebUploader.create({
            ...
            duplicate: true,
            // swf文件路径
            swf: yxin.WebUploader_swf,
            // 文件接收服务端。
            server: yxin.file_upload_url,
            // 选择文件的按钮。可选。
        ...
        uploader.on('error', function (type) {
            switch (type) {
                case 'Q_TYPE_DENIED':
                    Yxin.notify('文件类型不正确，只允许上传后缀名为：'+$ext+'，请重新上传！', 'danger');
                    break;
                case 'F_EXCEED_SIZE':
                    Yxin.notify('文件不得超过'+ ($size/1024) +'kb，请重新上传！', 'danger');
                    break;
            }
        });
    ...
    $('.js-upload-image,.js-upload-images').each(function () {
        ...
        uploader.on('error', function (type) {
            switch (type) {
                case 'Q_TYPE_DENIED':
                    Yxin.notify('图片类型不正确，只允许上传后缀名为：'+$ext+'，请重新上传！', 'danger');
                    break;
                case 'F_EXCEED_SIZE':
                    Yxin.notify('图片不得超过'+ ($size/1024) +'kb，请重新上传！', 'danger');
                    break;
            }
        });
    ...
    $('.select-linkage').change(function(){
        ...
        if (value !== '') {
            ...
            .fail(function(res) {
                Yxin.loading('hide');
                Yxin.notify($(res.responseText).find('h1').text() || '数据请求失败~', 'danger');
            });
        }
    });
    ...
    if (yxin.triggers != '') {
        /* 依赖显示 */
        // 先隐藏依赖项
        var $field_hide   = yxin.field_hide.split(',') || [];
        var $field_values = yxin.field_values.split(',') || [];
        ...
        $.each(yxin.triggers, function (trigger, content) {
                ...
                $(content).each(function () {
                    ...
                    if ($.inArray($value, $values) >= 0) {
                        ...
                        for (var item in $targets) {
                            ...
                            if (yxin._field_clear[trigger] !== undefined && yxin._field_clear[trigger] === 1) {
