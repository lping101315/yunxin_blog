...
jQuery(document).ready(function() {
    if ($.fn.editable) {
        // 快速编辑的url提交地址
        $.fn.editable.defaults.url = yxin.quick_edit_url;
        // 值为空时显示的信息
        ...
        $.fn.editable.defaults.params = function (params) {
            ...
            params.validate = yxin.validate;
            params.validate_fields = yxin.validate_fields;
            return params;
        };
        // 提交成功时的回调函数
    ...
    var goto = function (url, _curr_params, remove_page) {
        ...
        if (remove_page && yxin.curr_params['page'] !== undefined) {
            delete yxin.curr_params['page'];
        }

        if ($.isEmptyObject(yxin.curr_params)) {
            params = jQuery.param(_curr_params);
        } else {
            $.extend(yxin.curr_params, _curr_params);
            params = jQuery.param(yxin.curr_params);
        }
    ...
    // 初始化搜索
    var search_field = yxin.search_field;
    ...
    $('.table-builder .field-filter').click(function () {
        var self             = $(this),
            ...
            $_filter         = yxin._filter,
            $_filter_content = yxin._filter_content,
            $_field_display  = yxin._field_display,
            $data  = {
        ...
        layer.open({
            ...
            success: function () {
                ...
                // 获取数据
                $.post(yxin.get_filter_list, $data).success(function(res) {
                    if (1 !== res.code) {
            ...
            yes: function () {
                ...
                goto(yxin.curr_url, _curr_params, true);
            }
        });
        return false;
    });
    ...
    $('.table-builder .switch input:checkbox').on('click', function () {
        ...
        // 发送ajax请求
        Yxin.loading();
        $.post(yxin.quick_edit_url, $data).success(function(res) {
            Yxin.loading('hide');
            if (res.code) {
                Yxin.notify(res.msg, 'success');
            } else {
    ...
    $('#go-page').on('keyup', function (e) {
        if (e.keyCode === 13) {
            ...
            goto(yxin.curr_url, _curr_params);
        }
    });
    ...
    $('#page-container').delegate('a.pop', 'click', function () {
        ...
        // 处理各种回调方法
        yxin.layer.success = yxin.layer.success ? window[yxin.layer.success] : null;
        yxin.layer.yes     = yxin.layer.yes ? window[yxin.layer.yes] : null;
        yxin.layer.cancel  = yxin.layer.cancel ? window[yxin.layer.cancel] : null;
        yxin.layer.end     = yxin.layer.end ? window[yxin.layer.end] : null;
        yxin.layer.full    = yxin.layer.full ? window[yxin.layer.full] : null;
        yxin.layer.min     = yxin.layer.min ? window[yxin.layer.min] : null;
        yxin.layer.max     = yxin.layer.max ? window[yxin.layer.max] : null;
        yxin.layer.restore = yxin.layer.restore ? window[yxin.layer.restore] : null;

        if ($layer !== undefined) {
            // 处理各种回调方法
            $layer.success = $layer.success ? window[$layer.success] : yxin.layer.success;
            $layer.yes     = $layer.yes ? window[$layer.yes] : yxin.layer.yes;
            $layer.cancel  = $layer.cancel ? window[$layer.cancel] : yxin.layer.cancel;
            $layer.end     = $layer.end ? window[$layer.end] : yxin.layer.end;
            $layer.full    = $layer.full ? window[$layer.full] : yxin.layer.full;
            $layer.min     = $layer.min ? window[$layer.min] : yxin.layer.min;
            $layer.max     = $layer.max ? window[$layer.max] : yxin.layer.max;
            $layer.restore = $layer.restore ? window[$layer.restore] : yxin.layer.restore;

            $.extend($options, yxin.layer, $layer);
        } else {
            $.extend($options, yxin.layer);
        }
