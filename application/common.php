...
if (!function_exists('get_file_path')) {
     ...
     */
    function get_file_path($id = 0,$type ='all')
    {

    	if(strlen($id)>15 && !is_int($id)){
            $path = $id;
    	}else{
        ...
        if (!$path) {
            $path =  config('public_static_path').'admin/img/none.png';
        }

        if(!preg_match('/http/',$id) && $type != 'nopre'){
...
if (!function_exists('get_file_path2')) {
    /**
     * 获取附件路径
     * @param int $id 附件id

     */
    function get_file_path2($id = 0)
    {
        $path = model('admin/attachment')->getFilePath($id);

        if (!$path) {
            return config('public_static_path').'admin/img/none.png';
        }elseif(!preg_match('/http/',$id)){
            $path=get_host().$path;
        }
        return $path;
    }
